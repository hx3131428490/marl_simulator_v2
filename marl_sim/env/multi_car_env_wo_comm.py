...
def _global_observation(states: List[CarState], goals: List[Tuple[float, float]]) -> np.ndarray:
    """
    构建一个简单的全局观测向量。
    包含所有智能体的当前位置、朝向和目标位置。
    Flattened: [x1, y1, th1, gx1, gy1, x2, y2, th2, gx2, gy2, ...]
    """
    vec: List[float] = []
...
class MultiCarEnv:
    ...
    def reset(self, *, seed: Optional[int] = None) -> ObsDict:
        ...
        # 4. 生成初始观测
        gobs = _global_observation(self.states, self.goals)
